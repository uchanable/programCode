...
class BluetoothViewModel: NSObject, ObservableObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    enum ScanStatus: String {
        case none = "Status : Unconnected"
        case scanning = "Status : Scanning"
        case failed = "Status : Scan Failed (Try Again)"
        case connected = "Status : Connected"
        case stop = "Status : Stop Scanning"
    }
    
    @Published var scanStatus: ScanStatus = .none

    
}


/* alert를 두개 띄우는 방법인데 전혀 효과가 없음. 따라서 초기화함
...
class BluetoothViewModel: NSObject, ObservableObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    // AlertType을 감싸는 구조체를 정의합니다. 이 구조체는 Identifiable 프로토콜을 준수합니다.
    struct AlertIdentifier: Identifiable {
        let type: AlertType
        
        // Identifiable 프로토콜 준수를 위한 id 프로퍼티입니다.
        // AlertType의 rawValue를 사용하여 각각의 케이스가 고유한 id를 갖도록 합니다.
        var id: AlertType { type }
    }
    ...
    /*
    func updateAlertType(to newType: AlertType){
        DispatchQueue.main.async{
            self.alertType = newType
        }
    }*/
    
    func handleScanFailure() {
            alertIdentifier = AlertIdentifier(type: .scanFailed)
        }
    
    func handleConnectToDevice() {
            alertIdentifier = AlertIdentifier(type: .connectToDevice)
        }
    ...
    func scanForDevices() {
        ...
        scanTimer = Timer.scheduledTimer(withTimeInterval: 5.0, repeats: false) { [weak self] _ in
            guard let self = self else { return }
            if !self.isConnected && self.alertIdentifier?.type != .connectToDevice {
                // 원하는 블루투스 디바이스를 찾지 못한 경우
                print("Bluetooth device detection failed.")
                self.stopScanning() // 스캔을 중지합니다.
